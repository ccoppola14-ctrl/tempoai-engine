...
import { generatePatternMessage } from './patterns';

function patternTypeToTriggerType(
  patternType: string
...
function determineRecommendationType(
  ...
  if (pattern.liftPercent > 0) {
    if (pattern.patternType === 'daypart') return 'timing';
    return 'promote';
  }
  return 'demote';
}

function determineChannels(pattern: PatternResult): string[] {
...
function generateDetailedMessage(pattern: PatternResult): string {
  ...
  switch (pattern.patternType) {
    ...
    default:
      return generatePatternMessage(pattern);
  }
}
...
export function generateRecommendations(
  ...
  currentConditions?: { temperature?: number; weather?: string; daypart?: string; dayOfWeek?: number }
): Recommendation[] {
  const recommendations: Recommendation[] = [];

  // Sort patterns by magnitude (absolute lift) and confidence
  const sorted = [...patterns].sort(
    (a, b) => Math.abs(b.liftPercent) * b.confidence - Math.abs(a.liftPercent) * a.confidence
  );
...
function checkIfActive(
  pattern: PatternResult,
  conditions?: { temperature?: number; weather?: string; daypart?: string; dayOfWeek?: number }
): boolean {
  if (!conditions) return false;

  switch (pattern.patternType) {
    case 'temperature': {
      if (conditions.temperature === undefined) return false;
      const cond = pattern.triggerCondition;
      if (cond === 'temp < 60') return conditions.temperature < 60;
      if (cond === 'temp 60-75') return conditions.temperature >= 60 && conditions.temperature < 75;
      if (cond === 'temp 75-85') return conditions.temperature >= 75 && conditions.temperature < 85;
      if (cond === 'temp > 85') return conditions.temperature >= 85;
      return false;
    }
    case 'weather':
    ...
    case 'day_of_week': {
      if (conditions.dayOfWeek === undefined) return false;
