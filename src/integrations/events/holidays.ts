...
export type EventImpactType =
  | 'major_holiday'    // +30-50%
  | 'local_sports'     // +15-25%
  | 'concert_nearby'   // +10-20%
  | 'school_event'     // +5-10%
  | 'minor_holiday'    // +10-20%
  | 'negative_impact'; // -10-20%

export interface CalendarEvent {
  name: string;
  date: string; // YYYY-MM-DD
  type: EventImpactType;
  impact_multiplier: number; // e.g. 1.40 = +40%
  description: string;
}
 ...
 */
function getEventsForYear(year: number): CalendarEvent[] {
 ...
 */
export function getEventImpactForDate(date: string): { multiplier: number; event: CalendarEvent | null } {
  ...
  const mostImpactful = events.reduce((best, e) =>
    Math.abs(e.impact_multiplier - 1) > Math.abs(best.impact_multiplier - 1) ? e : best,
  );
 ...
 */
export const EVENT_TYPE_RANGES: Record<EventImpactType, { min: number; max: number; label: string }> = {
