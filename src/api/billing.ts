...
router.post('/create-checkout', async (req: Request, res: Response) => {
  ...
  try {
    ...
    logger.info('Billing', `Checkout session created for merchant ${merchantId} (${plan})`);
...
router.get('/success', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (merchantId) {
      ...
      const square = await prisma.squareMerchant.findUnique({ where: { merchantId } });
      if (square) {
        ...
        const clover = await prisma.cloverMerchant.findUnique({ where: { merchantId } });
        if (clover) {
      ...
      logger.info('Billing', `Subscription activated for merchant ${merchantId} (${plan})`);
    }

    res.redirect(`${dashboardUrl}/settings?tab=billing&status=success`);
  } catch (err) {
    logger.error('Billing', 'Failed to verify checkout session', err);
    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (merchantId) {
          ...
          logger.info('Billing', `Webhook: checkout completed for ${merchantId}`);
        }
        break;
      }
      ...
      case 'customer.subscription.deleted': {
        ...
        if (merchantId) {
          ...
          logger.info('Billing', `Webhook: subscription cancelled for ${merchantId}`);
        }
        break;
      }
      ...
      case 'invoice.payment_failed': {
        ...
        if (subscriptionId) {
          ...
          const square = await prisma.squareMerchant.findFirst({
          ...
          if (square) {
            ...
            const clover = await prisma.cloverMerchant.findFirst({
              where: { stripeSubscriptionId: subscriptionId },
            });
            if (clover) {
              await prisma.cloverMerchant.update({
          ...
          logger.info('Billing', `Webhook: payment failed for subscription ${subscriptionId}`);
        }
        break;
      }
...
router.get('/status', async (req: Request, res: Response) => {
  ...
  try {
    ...
    logger.error('Billing', 'Failed to get billing status', err);
...
async function updateMerchantBilling(
  ...
  const square = await prisma.squareMerchant.findUnique({ where: { merchantId } });
  if (square) {
    await prisma.squareMerchant.update({ where: { merchantId }, data });
    return;
  }

  const clover = await prisma.cloverMerchant.findUnique({ where: { merchantId } });
  if (clover) {
    await prisma.cloverMerchant.update({ where: { merchantId }, data });
  }
}
