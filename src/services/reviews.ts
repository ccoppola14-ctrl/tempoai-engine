 ...
 */
export function generateDraftResponse(input: DraftResponseInput): string {
 ...
 */
export async function saveReviewResponse(reviewId: string, responseText: string) {
  try {
    ...
    logger.warn('Reviews', `Could not save response for review ${reviewId}: ${err instanceof Error ? err.message : String(err)}`);
    return null;
  }
}
