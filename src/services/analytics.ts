 ...
 */
async function avgDailyRevenue(
  locationId: string,
  from: Date,
  to: Date,
): Promise<{ avgDaily: number; totalDays: number; totalRevenue: number }> {
  ...
  if (orders.length === 0) return { avgDaily: 0, totalDays: 0, totalRevenue: 0 };
...
export async function getBeforeAfterRevenue(locationId: string): Promise<BeforeAfterResult> {
  ...
  const preInstallOrders = await prisma.order.count({
    where: {
      locationId,
  ...
  let confidence = 0.3;
  if (totalDataDays >= 60) confidence = 0.95;
  else if (totalDataDays >= 45) confidence = 0.85;
  else if (totalDataDays >= 30) confidence = 0.75;
  else if (totalDataDays >= 14) confidence = 0.6;
  else if (totalDataDays >= 7) confidence = 0.45;

  const estimatedAnnualImpact = Math.max(0, liftDollars * 12);

  logger.info('Analytics', `Before/After for ${location.name}: ${liftPercent.toFixed(1)}% lift`);
...
export async function getAttribution(locationId: string): Promise<AttributionResult> {
  const location = await prisma.location.findUniqueOrThrow({ where: { id: locationId } });
  ...
  const topPerforming: TopRecommendation[] = applied
 ...
 */
export async function getBeforeAfterSnippet(locationId: string): Promise<string | null> {
  try {
    const result = await getBeforeAfterRevenue(locationId);
    if (result.days_after < 2) return null;
