 ...
 */
async function fetchWeatherForecast(city: string): Promise<WeatherForecastDay[]> {
  try {
    ...
    logger.warn('Forecast', `Weather fetch failed: ${err instanceof Error ? err.message : String(err)}`);
    return [];
  }
}
 ...
 */
function estimateWeatherImpact(condition: string, temperature: number): number {
  const lowerCond = condition.toLowerCase();
  let impact = 0;

  // Bad weather reduces sales
  if (lowerCond.includes('rain') || lowerCond.includes('drizzle')) impact -= 0.10;
  if (lowerCond.includes('snow') || lowerCond.includes('blizzard')) impact -= 0.20;
  if (lowerCond.includes('thunder') || lowerCond.includes('storm')) impact -= 0.15;

  // Good weather boosts sales
  if (lowerCond.includes('sunny') || lowerCond.includes('clear')) impact += 0.05;

  // Extreme temperatures reduce sales
  if (temperature < 25) impact -= 0.15;
  else if (temperature < 40) impact -= 0.05;
  else if (temperature > 95) impact -= 0.10;
 ...
 */
function getStaffingRecommendation(predictedSales: number, dayName: string): { recommended: number; reason: string } {
  ...
  if (isWeekend) reason += ` (weekend +1 staff)`;
  if (predictedSales < 500) reason += '. Light day — consider reduced hours.';
  else if (predictedSales > 2000) reason += '. Heavy day — consider extra prep staff.';
 ...
 */
export async function generateForecast(locationId: string): Promise<DayForecast[]> {
  logger.info('Forecast', `Generating forecast for location ${locationId}`);
  ...
  const dayOfWeekStats: Record<number, { totalSales: number; totalOrders: number; count: number }> = {};
  for (let d = 0; d < 7; d++) {
  ...
  for (const order of orders) {
    ...
    if (!dailyTotals[dateKey]) dailyTotals[dateKey] = { sales: 0, orders: 0 };
  ...
  for (let i = 1; i <= 7; i++) {
    ...
    let confidence = 0.5;
    if (stats.count >= 12) confidence = 0.9;
    else if (stats.count >= 8) confidence = 0.8;
    else if (stats.count >= 4) confidence = 0.7;
    else if (stats.count >= 1) confidence = 0.6;
    if (!weatherDay) confidence -= 0.05;
  ...
  logger.info('Forecast', `Generated 7-day forecast for ${locationId}`);
  return forecasts;
}
