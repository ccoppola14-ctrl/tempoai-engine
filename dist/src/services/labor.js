...
exports.analyzeLaborEfficiency = analyzeLaborEfficiency;
exports.generateStaffingRecommendation = generateStaffingRecommendation;
exports.generateWeeklyLaborPlan = generateWeeklyLaborPlan;
exports.calculateLaborWaste = calculateLaborWaste;
exports.seedDefaultLaborTargets = seedDefaultLaborTargets;
...
const LABOR_DAYPARTS = ['breakfast', 'lunch', 'dinner', 'late_night'];
...
function getDaypartHours(daypart) {
    const dp = dayparts_1.DAYPARTS.find(d => d.name === daypart);
    if (!dp)
        return 3;
    if (dp.endHour > dp.startHour)
        return dp.endHour - dp.startHour;
    return (24 - dp.startHour) + dp.endHour; // wraps midnight
}
function classifyOrderDaypart(timestamp) {
    const hour = timestamp.getHours();
    const dp = (0, dayparts_1.getDaypart)(hour);
 ...
 */
async function analyzeLaborEfficiency(locationId, days = 30) {
    logger_1.logger.info('Labor', `Analyzing labor efficiency for ${locationId} (${days} days)`);
    const since = new Date();
    since.setDate(since.getDate() - days);
    const [shifts, orders, targets] = await Promise.all([
        client_1.default.staffShift.findMany({
        ...
        client_1.default.laborTarget.findMany({ where: { locationId } }),
    ]);
    ...
    const revenueByDateDaypart = new Map();
    ...
    const shiftsByDateDaypart = new Map();
    for (const shift of shifts) {
        const date = shift.startTime.toISOString().split('T')[0];
        const dp = classifyOrderDaypart(shift.startTime);
        const key = `${date}-${dp}`;
        if (!shiftsByDateDaypart.has(key))
            shiftsByDateDaypart.set(key, []);
        shiftsByDateDaypart.get(key).push(shift);
    }
    // Collect all unique dates
    const allDates = new Set();
    for (const order of orders)
        allDates.add(order.timestamp.toISOString().split('T')[0]);
    for (const shift of shifts)
        allDates.add(shift.startTime.toISOString().split('T')[0]);
    const sortedDates = [...allDates].sort();
    let totalRevenue = 0;
    let totalLaborCost = 0;
    let totalWastedCost = 0;
    let totalMissedRevenue = 0;
    const daypartAggregates = {};
    ...
    for (const date of sortedDates) {
        ...
        for (const dp of LABOR_DAYPARTS) {
            ...
            const optimalStaff = dpRevenue > 0
                ? Math.max(target?.minStaff ?? DEFAULT_TARGETS[dp]?.minStaff ?? 2, Math.min(target?.maxStaff ?? DEFAULT_TARGETS[dp]?.maxStaff ?? 10, Math.ceil(dpRevenue / (revenuePerHour * dpHours))))
            ...
            const analysis = {
                daypart: dp,
                label: (0, dayparts_1.getDaypartLabel)(dp),
                staffCount,
                laborHours,
                laborCost,
                revenue: dpRevenue,
        ...
        if (dayRevenue > 0 && dayLaborCost > 0) {
            if (!bestDay || dayLaborPct < bestDay.laborPct)
                bestDay = { date, laborPct: Math.round(dayLaborPct * 10) / 10 };
            if (!worstDay || dayLaborPct > worstDay.laborPct)
                worstDay = { date, laborPct: Math.round(dayLaborPct * 10) / 10 };
        }
    }
    const totalLaborPct = totalRevenue > 0 ? (totalLaborCost / totalRevenue) * 100 : 0;
    const daysWithData = sortedDates.length || 1;
    const byDaypart = LABOR_DAYPARTS.map(dp => {
 ...
 */
async function generateStaffingRecommendation(locationId, targetDate) {
    logger_1.logger.info('Labor', `Generating staffing recommendation for ${locationId} on ${targetDate}`);
    const targetDateObj = new Date(targetDate + 'T12:00:00Z');
    const dow = targetDateObj.getDay();
    const dayName = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][dow];
    // Get labor targets
    const targets = await client_1.default.laborTarget.findMany({
        where: { locationId, dayOfWeek: dow },
    });
    const targetMap = new Map();
    for (const t of targets)
        targetMap.set(t.daypart, t);
    ...
    // Get event impact
    const eventImpact = (0, events_1.getEventImpactForDate)(targetDate);
    ...
    const historicalOrders = await client_1.default.order.findMany({
        where: {
            locationId,
    ...
    const existingShifts = await client_1.default.staffShift.findMany({
    ...
    for (const dp of LABOR_DAYPARTS) {
        ...
        const dpDef = dayparts_1.DAYPARTS.find(d => d.name === dp);
        ...
        // Build reasoning
        const reasons = [];
        reasons.push(`Predicted ${(0, dayparts_1.getDaypartLabel)(dp).toLowerCase()} revenue: $${Math.round(predictedDpRevenue)}`);
        ...
        if (currentScheduled !== null && delta !== null) {
            if (delta > 0)
                reasons.push(`Currently ${Math.abs(delta)} staff short`);
            else if (delta < 0)
                reasons.push(`Currently ${Math.abs(delta)} staff over — save ~$${Math.round(estimatedSavings)}`);
            else
                reasons.push('Staffing looks optimal');
        }
        dayparts.push({
            daypart: dp,
            label: (0, dayparts_1.getDaypartLabel)(dp),
 ...
 */
async function generateWeeklyLaborPlan(locationId, startDate) {
    logger_1.logger.info('Labor', `Generating weekly labor plan for ${locationId} starting ${startDate}`);
    const days = [];
 ...
 */
async function calculateLaborWaste(locationId, days = 30) {
    logger_1.logger.info('Labor', `Calculating labor waste for ${locationId} (${days} days)`);
    ...
    for (const day of analysis.dailyBreakdown) {
        for (const dp of day.dayparts) {
            if (dp.wastedCost > 0 || dp.missedRevenue > 0) {
                try {
                    await client_1.default.laborRecommendation.create({
                        data: {
                            locationId,
                            date: day.date,
                            daypart: dp.daypart,
 ...
 */
async function seedDefaultLaborTargets(locationId) {
    logger_1.logger.info('Labor', `Seeding default labor targets for ${locationId}`);
    let count = 0;
    for (let dow = 0; dow < 7; dow++) {
        for (const [dp, defaults] of Object.entries(DEFAULT_TARGETS)) {
            try {
                await client_1.default.laborTarget.upsert({
                    where: {
    ...
    logger_1.logger.info('Labor', `Seeded ${count} labor targets for ${locationId}`);
    return count;
}
