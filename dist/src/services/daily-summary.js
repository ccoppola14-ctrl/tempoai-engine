...
function generatePromoSuggestion(itemName, triggerType, triggerCondition) {
...
async function generateDailySummary(locationId, date = new Date()) {
    ...
    const latestWeather = weatherSnapshot ?? await client_1.default.weatherSnapshot.findFirst({
        where: { locationId },
        orderBy: { timestamp: 'desc' },
    });
    ...
    try {
        ...
        for (const rec of activeRecs) {
            if (promoRecommendations.length >= 3)
                break;
            let matches = false;
            switch (rec.triggerType) {
                case 'temperature':
                    if (temperature !== null) {
                        if (rec.triggerCondition === 'temp < 60')
                            matches = temperature < 60;
                        else if (rec.triggerCondition === 'temp 60-75')
                            matches = temperature >= 60 && temperature < 75;
                        else if (rec.triggerCondition === 'temp 75-85')
                            matches = temperature >= 75 && temperature < 85;
                        else if (rec.triggerCondition === 'temp > 85')
                            matches = temperature >= 85;
                    }
                    break;
                case 'weather':
        ...
        logger_1.logger.warn('DailySummary', `Failed to get promo recommendations for ${location.name}`, err);
    }
    // ─── UPSELL TIP (from AI patterns — item correlations) ───
    let upsellTip = null;
    try {
        // Find the highest-lift pairing pattern
        const pairPattern = await client_1.default.aIPattern.findFirst({
        ...
        if (!upsellTip && topItems.length > 0) {
            ...
            const allItems = await client_1.default.menuItem.findMany({
            ...
            const addon = allItems.find(i => i.name !== topItems[0].name &&
        ...
        logger_1.logger.warn('DailySummary', `Failed to get upsell tip for ${location.name}`, err);
    }
    // ─── STAFFING NOTE (weather + forecast impact on traffic) ───
    let staffingNote = null;
    try {
        if (latestWeather) {
            const cond = latestWeather.conditions.toLowerCase();
            const temp = latestWeather.temperature;
        ...
        logger_1.logger.warn('DailySummary', `Failed to get staffing note for ${location.name}`, err);
    }
    ...
    try {
        if (orderCount > 0) {
            ...
            const recentItems = await client_1.default.orderItem.groupBy({
                by: ['menuItemId'],
                where: {
                    order: {
                        locationId,
            ...
            for (const ri of recentItems) {
                ...
                if (avgDaily < 2)
                    continue; // Skip items with very low volume
                ...
                if (todayQty < avgDaily * 0.3 && avgDaily >= 3) {
                    const menuItem = await client_1.default.menuItem.findUnique({ where: { id: ri.menuItemId } });
                    if (menuItem) {
        ...
        logger_1.logger.warn('DailySummary', `Failed to get underperformers for ${location.name}`, err);
    }
