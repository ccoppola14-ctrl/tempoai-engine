...
function simulateTampaWeather(date) {
...
// ─── Order generation ────────────────────────────────────
function getItemSalesMultiplier(item, weather, hour, dayOfWeek) {
    let multiplier = 1.0;
    // Temperature effects
...
function generateOrdersForHour(menuItems, weather, hour, dayOfWeek) {
    const orders = [];
    // Base order volume by hour
    let baseOrders;
    if (hour >= 7 && hour < 9)
        baseOrders = randomInt(8, 18); // Breakfast rush
    else if (hour >= 9 && hour < 11)
        baseOrders = randomInt(5, 12);
    else if (hour >= 11 && hour < 14)
        baseOrders = randomInt(15, 30); // Lunch rush
    else if (hour >= 14 && hour < 17)
        baseOrders = randomInt(3, 8); // Afternoon lull
    else if (hour >= 17 && hour < 20)
        baseOrders = randomInt(12, 25); // Dinner
    else if (hour >= 20 && hour < 22)
        baseOrders = randomInt(4, 10);
    else
        baseOrders = randomInt(0, 3);
    // Weekend boost
